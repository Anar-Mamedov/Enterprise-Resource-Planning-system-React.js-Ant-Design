...
import { Form, Drawer, Row, message, Divider, Button, Input, Col, Typography, Select, DatePicker, Tabs, Collapse, Space, Card, Switch, Tag, Tooltip } from 'antd';
import { Languages } from '../../../Config';
import { PlusOutlined, CloseOutlined, RightSquareOutlined } from '@ant-design/icons';
...
const Index = ({ onClose, open, loading, setLoading, updateId, setUpdateId }) => {
   ...
   const [currentStatus, setCurrentStatus] = useState({ key: 'draft', name: 'Gözləmədə' });
   //*Piclist Start ---------------------------------------------------------------------------------------
   ...
   const getShowApi = () => {
      if (updateId) {
         API.Acc.FacturaContract.show(updateId).then(res => {
            setLoading(true)
            if (res.status === 200) {
               setLoading(false)
               let show = res.data.data
               setItem([{ name: 'uuid', value: show?.uuid, errors: null }]);
               setItem([{ name: 'prediment_id', value: show?.prediment?.uuid, errors: null }]);
               setItem([{ name: 'company_id', value: show?.company?.uuid, errors: null }]);
               setItem([{ name: 'supplier_id', value: show?.supplier?.uuid, errors: null }]);
               setItem([{ name: 'contract_type_id', value: show?.contract_type?.uuid, errors: null }]);
               setItem([{ name: 'currency_type_id', value: show?.currency?.uuid, errors: null }]);
               ...
               form.setFields([{ name: 'payment_type_id', value: show?.payment_type?.uuid, errors: null }]);
               ...
               show?.items?.forEach((item, index) => {
                  setItem([{ name: ['items', index, 'uuid'], value: item?.uuid, errors: null }]);
                  setItem([{ name: ['items', index, 'product_type'], value: item?.product_type, errors: null }]);
                  setItem([{ name: ['items', index, 'product_id'], value: item?.product?.uuid, errors: null }]);
                  setItem([{ name: ['items', index, 'description_id'], value: item?.description?.name, errors: null }]);
                  setItem([{ name: ['items', index, 'unit_id'], value: item?.unit?.uuid, errors: null }]);
                  ...
                  setItem([{ name: ['items', index, 'tax_id'], value: item?.tax?.uuid, errors: null }]);
                  ...
                  setItem([{ name: ['items', index, 'bank_guarantee_id'], value: item?.bank_guarantee?.uuid, errors: null }]);
                  item?.bank_guarantee_date && setItem([{ name: ['items', index, 'bank_guarantee_date'], value: dayjs(item?.bank_guarantee_date), errors: null }]);
                  setItem([{ name: ['items', index, 'bank_guarantee_time_id'], value: item?.bank_guarantee_time?.uuid, errors: null }]);
               ...
               // !----------------------------------------
               let temp = show?.items.map((el) => {
                  return {
                     ...el, status: true, collapse: false, servisProducts: serviceProds, productProducts: products, descriptions: []
                  }
               })
               show?.items?.forEach((el, index) => {
                  API.Warehouse.productAtributeValue.crossCombinations({ product_id: el?.product?.uuid }).then(res => {
                     temp[index].descriptions = res.data.data
                  });
                  let hasil = (el?.amount ?? 0) * (el?.quantity ?? 0)
                  let hasilEdv = ((el?.amount ?? 0) * (el?.quantity ?? 0) * (el?.tax?.tax_value || 0)) / 100
                  sumTotalAmount = (sumTotalAmount || 0) + hasil;
                  sumEDVAmount = (sumEDVAmount || 0) + hasilEdv;
               })
               setTabInfoPanel([...temp])
   ...
   const onFinish = (value) => {
      ...
      if (isReqCollapse) {
         ...
         setLoading(true)
         API.Acc.FacturaContract.update(updateId, newValue).then(res => {
            if (res.status === 400) {
               setLoading(false)
               const messages = res.data.data;
               for (let key in messages) setFormError(key, messages[key]);
               Object.keys(messages).forEach((a, i) => {
                  if (a.split('.').includes('items')) {
                     tabInfoPanel[a.split('.')[1]].collapse = true;
                  }
               })
   ...
   const handleCategoryType = (value, index) => {
      setLoading(true)
      tabInfoPanel[index].descriptions = []
   ...
   const getProducts = () => {
      setLoading(true);
      API.Acc.FacturaContract.productCategory({ 'item_type': 'product' }).then(res => {
         setLoading(false);
      ...
      if (value !== undefined) {
         API.Warehouse.productAtributeValue.crossCombinations(uuid).then(res => {
            if (res.status === 400) {
               ...
               setLoading(false);
               tabInfoPanel[index].descriptions = res.data.data;
               setTabInfoPanel([...tabInfoPanel]);
            }
         });
      } else {
   ...
   //*--------------
   //! Delete Api Start------------------------------------------------------------------------------------
   const deleteMaliyyeCollapse = (e, tabInfo, index) => {
      e.stopPropagation();
      if (tabInfo?.uuid) {
         API.Acc.FacturaContract.deleteItem(tabInfo.uuid).then(res => {
            console.log(res.status);
            if (res.status === 200) {
               message.success('Maliyyə detalı silindi')
            }
         })
      }
                  ...
                  <Panel key={index + 1} showArrow={false} onClick={() => {
                     ...
                     <span style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Space>
                           ...
                           <Button type="primary" danger onClick={(e) => deleteMaliyyeCollapse(e, tabInfo, index)}> - </Button>
                        </Space>
                     </span>}>
                     <Row style={{ width: '100%', height: '125px', display: 'flex' }} gutter={14}>
                        <Col span={5}>
                           <Form.Item hidden name={['items', index, 'uuid']} />
                              ...
                              {tabInfoPanel[index].product_type === 'service' &&
                                 <Form.Item className='c-form-item' name={['items', index, 'product_id']} rules={[{ required: false, message: innerText.error_insert_data }]}>
                                    <Select
                                       ...
                                       {tabInfoPanel?.[index]?.servisProducts?.map((ser, index) => (
                              ...
                              {tabInfoPanel[index].product_type === 'product' &&
                                 <Form.Item className='c-form-item' name={['items', index, 'product_id']} rules={[{ required: false, message: innerText.error_insert_data }]}>
                                    <Select
                                       ...
                                       {tabInfoPanel?.[index]?.productProducts?.map((ser, index) => (
                                          <Option className='c-select-option' key={index} value={ser?.uuid}>{ser?.name}</Option>
                                       ))}
                                    </Select>
                                 </Form.Item>
                        ...
                        <Col span={5}>
                           <Form.Item className='c-form-item' label='Təsnifat' name={['items', index, 'description_id']} rules={[{ required: false, message: innerText.error_insert_data }]}>
                              ...
                              >
                                 {tabInfoPanel?.[index]?.descriptions?.map((desc, index) => (
               ...
               <Row gutter={12}>
                  <Col span={24} >
                     <Button block onClick={() => {
                        tabInfoPanel.push({
                           status: true, collapse: true, product_type: 'service', servisProducts: serviceProds, productProducts: products, descriptions: [], checkEdv: false
                        })
                        setTabInfoPanel([...tabInfoPanel])
                     }} type="dashed">
                        +
   ...
   return (
      <Drawer
         ...
         extra={
            <Space>
               <Tooltip title='Ətraflı' arrow={mergedArrow}>
                  <Link to={`show/${updateId}`}><Tag style={{ cursor: 'pointer' }} color='volcano'><RightSquareOutlined /></Tag></Link>
               </Tooltip>
               <Tag onClick={() => { setShowAllDisabled(prev => !prev) }} color='#28C76F'>{showAllDisabled ? 'Göstər' : 'Gizlət'}</Tag>
