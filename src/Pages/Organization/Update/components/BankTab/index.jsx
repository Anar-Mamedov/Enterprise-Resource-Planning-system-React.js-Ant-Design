...
const Index = ({ bankTime, setItem, form, delivers, setDelivers }) => {
  ...
  const getBankHandle = (value, index, resetBranch = true) => {
    if (resetBranch) {
      form.setFieldValue(["bank", index, "branch"], null);
    }
  ...
  useEffect(() => {
    ...
    delivers.forEach((deliver, index) => {
      getBankHandle(deliver.id, index, false);
      selectBranch(deliver.branchId, index);
      selectCurrency(deliver.currencyId, index);
    });
  }, []);
    ...
    <>
          ...
          delive && (
            <Row key={index}>
              <Col span={24}>
                <Row gutter={14}>
                  <Col span={5}>
                    <Form.Item hidden name={["bank", index, "uuid"]} />
      ...
      <Row gutter={14}>
        <Col span={24} align="middle">
          <Button icon={<PlusOutlined />} onClick={() => setDelivers((prev) => [...prev, { isChecked: false, id: null, branchId: null, currencyId: null, bankBranchId: [], description: [] }])} />
        </Col>
      </Row>
    </>
  );
