...
const Index = () => {
  ...
  const columns = [
    {
      title: <Sorter title="Nömrə" onClick={() => Sort(0)} />,
      dataIndex: "number",
      key: "number",
      ellipsis: false,
      width: 170,
    ...
    {
      title: <Sorter title="Təşkilat" onClick={() => Sort(1)} />,
      dataIndex: "company",
      key: "company",
      ellipsis: false,
      width: 170,
      ...
      render: (text) => <> {text?.name_nat}</>,
    },
    {
      title: <Sorter title="Kontragent" onClick={() => Sort(2)} />,
      dataIndex: "supplier",
      key: "supplier",
      ellipsis: false,
      width: 170,
    ...
    {
      title: <Sorter title="Növ" onClick={() => Sort(3)} />,
      dataIndex: "type",
      key: "type",
      ellipsis: false,
      width: 170,
    ...
    {
      title: <Sorter title="Tarix" onClick={() => Sort(3)} />,
    ...
    {
      title: <Sorter title="Predmet" onClick={() => Sort(3)} />,
      dataIndex: "predment",
      key: "predment",
      ellipsis: false,
      width: 170,
    ...
    {
      title: <Sorter title="Ödəniş növü" onClick={() => Sort(4)} />,
    ...
    {
      title: <Sorter title="Başlama tarixi" onClick={() => Sort(5)} />,
    ...
    {
      title: <Sorter title="Bitmə tarixi" onClick={() => Sort(6)} />,
      dataIndex: "end_date",
      key: "end_date",
      ellipsis: false,
      width: 170,
  ...
  const renderTableView = () => {
    ...
    API.Procurement.Contracts.Contract.index(JSON.stringify(showdata.current)).then((res) => {
      setLoading(true);
  ...
  const changeStatus = (status) => {
    if (selectedRowKeys.length) {
      ...
      API.Procurement.Contracts.Contract.activity(params).then((res) => {
        if (res.status === 200) {
  ...
  //   console.log("HGDFHSDGFHDHFGHDFG", setShowTableData);
  return (
    <Layout className="c-wrapper">
      <Content style={{ position: "relative" }}>
        {selectedRowKeys.length > 0 && (
          <Row className="status-box">
            <Col>
              <Text className="status-box__text">
                {selectedRowKeys.length} {innerText.data_selected}
              </Text>
            </Col>
            <Col>
        ...
        <Row className="c-table-header" justify="space-between" align="middle">
          ...
          <Col>
            <Space>
              ...
              <Row>
                <Button onClick={(_) => changeView("active")} className={"active-button" + " " + (activeView === "active" ? "active" : "")}>
                  {innerText.btn_active}
                </Button>
                <Button onClick={(_) => changeView("deactive")} className={"deactive-button" + " " + (activeView === "deactive" ? "active" : "")}>
                  {innerText.btn_deactive}
                </Button>
              </Row>
              <Button className="c-btn c-btn--icon c-btn--excel">
                <FileExcelOutlined />
              </Button>
              <Button className="c-btn c-btn--icon c-btn-outline-secondary">
                <PrinterOutlined />
              </Button>
              <Button className="c-btn c-btn--purple" onClick={clearFilter} icon={<ClearOutlined />}>
                {innerText.btn_clear_filter}
              </Button>
              <Button className="c-btn c-btn--primary" type="primary" onClick={showDrawer} icon={<PlusOutlined />}>
                {innerText.btn_new}
              </Button>
            </Space>
          </Col>
      ...
      <Insert renderTableView={renderTableView} open={open} onClose={onClose} filters={showdata.current} loading={loading} setLoading={setLoading} />
