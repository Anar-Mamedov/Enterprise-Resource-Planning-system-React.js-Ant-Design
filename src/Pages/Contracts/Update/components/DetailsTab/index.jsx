...
const Index = ({ taxEdv, form, setItem, detailstab, setDetailstab }) => {
  ...
  const getTesnifatHandle = (value, index) => {
    ...
    API.Warehouse.productAtributeValue.crossCombinations({ product_id: value }).then((res) => {
      detailstab[index].descriptions = res.data.data;
  ...
  useEffect(() => {
    results = 0;
    taxTotal = 0;
    detailstab?.forEach((details, index) => {
      // valArrEDVAmount[index] = ((details?.quantity ?? 0) * (details?.amount ?? 0) * (details?.tax?.name ?? 0)) / 100;
      // sumTotalAmount = valArrTotalAmount.filter((a) => a).reduce((accumulator, currentValue) => accumulator + currentValue, 0);
      taxTotal = ((details?.quantity ?? 0) * (details?.amount ?? 0) * (details?.tax?.name ?? 0)) / 100 + taxTotal;
      results = (details?.quantity ?? 0) * (details?.amount ?? 0) + results;
    });
    ...
    <>
          ...
          detailtab && (
            <Row key={index}>
              <Col span={24}>
                <Row gutter={14}>
                  <Col span={5}>
                    <Form.Item hidden name={["contract_details", index, "uuid"]} />
                  ...
                  <Col span={5}>
                    <Form.Item className="c-form-item" label="Təsnifat" name={["contract_details", index, "description_id"]} rules={[{ required: false, message: "Zəhmət olmasa daxil edin" }]}>
                      <Select className="c-select" allowClear showSearch filterOption={(input, option) => (option?.children.toLowerCase() ?? "").includes(input.toLowerCase())} placeholder="Secim edin">
                        {detailstab?.[index]?.descriptions?.map((a, i) => (
                          <Option className="c-select-option" key={i} value={a.uuid}>
                            {a.name}
                          </Option>
                        ))}
                        ...
                        >
                          ...
                          <Switch
                            onChange={(val) => {
                              ...
                              detailstab[index].is_checked = val;
                              setDetailstab([...detailstab]);
                            }}
                            size="small"
                            checked={detailstab?.[index]?.is_checked}
                          />
                        </span>
                      }
                    >
                      {detailstab?.[index]?.is_checked ? (
                        <Space size={0}>
      ...
      <Row gutter={14}>
        <Col span={24} align="middle">
          <Button icon={<PlusOutlined />} onClick={() => setDetailstab((prev) => [...prev, { is_checked: false, descriptions: [] }])} />
        </Col>
      </Row>
      <Space>
