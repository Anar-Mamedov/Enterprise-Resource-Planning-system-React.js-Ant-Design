...
const Index = ({ bankTime, taxEdv, banks, setItem, form }) => {
  ...
  const financeDetailSwich = (val, index) => {
    ...
    details[index].isChecked = val;
    setDetails([...details]);
    // form.getFieldValue(["finance_details", index, "tax_id"]);
  };
  const calculateHandle = (e, index, name) => {
    if (Number(e.target.value) < 0) {
      setItem([{ name: ["finance_details", name], value: 0, errors: null }]);
      return;
    }
    if (e.target.value.startsWith("0")) {
      setItem([{ name: ["finance_details", name], value: null, errors: null }]);
      return;
    }
    const filtered = taxEdv.filter((a) => a.uuid === form.getFieldValue(["finance_details", "tax_id"]) && a.name);
    setItem([{ name: ["finance_details", "tax_id_value"], value: (e.target.value * (filtered?.[0]?.name ?? 0)) / 100, errors: null }]);
  };
  const handleCalcBillEdv = (e, val, index) => {
    if (val === undefined) {
      setItem([{ name: ["finance_details", "tax_id_value"], value: 0, errors: null }]);
    } else {
      let amount = form.getFieldValue(["finance_details", "amount"]) || 0;
      setItem([{ name: ["finance_details", "tax_id_value"], value: (amount * val.children[0]) / 100, errors: null }]);
    }
  };
  return (
    <>
                ...
                <Row gutter={14}>
                  <Col span={5}>
                    <Form.Item
                      ...
                      label="Məbləğ"
                      name={["finance_details", "amount"]}
                      rules={[
                        {
                          required: false,
                          message: "Zehmet olmasa daxil edin",
                        },
                      ]}
                    >
                      ...
                      label={
                        ...
                        >
                          ...
                          <Switch
                            ...
                            size="small"
                            checked={details?.[index]?.isChecked}
                          />
                        </span>
                      }
                    >
                      {details?.[index]?.isChecked ? (
                        <Space size={0}>
                          <Form.Item
                            className="c-form-item"
                            name={["finance_details", "tax_id"]}
                            rules={[
                              {
                                required: true,
                          ...
                          <Form.Item
                            className="c-form-item"
                            name={["finance_details", "tax_id_value"]}
                            rules={[
                              {
                                required: false,
                                message: "ƏDV-dən azad!",
                        ...
                        <Space>
                          ...
                          <Form.Item hidden name={["finance_details", "tax_id"]} />
                          <Form.Item hidden name={["finance_details", "tax_id_value"]} />
                        </Space>
                      )}
                    </Form.Item>
                  </Col>
                  <Col span={5}>
                    <Form.Item
                      ...
                      name={["finance_details", "bank_guarantee_amount"]}
                      rules={[
                        {
                          required: false,
                  ...
                  <Col span={5}>
                    <Form.Item
                      ...
                      name={["finance_details", "bank_guarantee_id"]}
                      rules={[
                        {
                          required: false,
                  ...
                  <Col span={5}>
                    <Form.Item
                      ...
                      name={["finance_details", "bank_guarantee_date"]}
                      rules={[
                        {
                          required: false,
                          message: "Zehmet olmasa secim edin",
                  ...
                  <Col span={5}>
                    <Form.Item label="Bank zəmanət müddəti">
                      <Space size={0}>
                        <Form.Item
                          className="c-form-item"
                          name={["finance_details", "duration_time_id"]}
                          rules={[
                            {
                              required: false,
                              message: "Zəhmət olmasa daxil edin",
                        ...
                        <Form.Item
                          className="c-form-item"
                          name={["finance_details", "bank_guarantee_date_duration"]}
                          rules={[
                            {
                              required: false,
                              message: "Zəhmət olmasa daxil edin",
